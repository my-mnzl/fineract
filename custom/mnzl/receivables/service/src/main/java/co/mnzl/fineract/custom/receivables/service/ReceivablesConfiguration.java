...
public class ReceivablesConfiguration {
    ...
    public String scopeKey(JsonNode scope) {
        ...
        for (String field : new String[]{"platformId", "financierOrganizationId", "environment", "ledgerEpoch"}) financier.put(field, text(scope, field));
        return json.hash(financier);
    }
    public String tenantId() { return ThreadLocalContextUtil.getTenant().getTenantIdentifier(); }
    ...
    public JsonNode configure(String request) {
        ...
        for (JsonNode entry : input.get("accountMap")) {
            ...
            var rows = store.jdbc().queryForList("select disabled, account_usage from acc_gl_account where id=?", Long.parseLong(text(entry, "nativeGlAccountId")));
        ...
        fields.put("scope_key", scope); fields.put("scope_json", json.write(input.get("scope")));
        fields.put("financier_id", text(input.get("scope"), "financierOrganizationId")); fields.put("epoch", text(input.get("scope"), "ledgerEpoch"));
        fields.put("integration_user_id", Long.parseLong(text(input, "integrationUserId"))); fields.put("product_id", Long.parseLong(text(input, "productId")));
        fields.put("office_id", Long.parseLong(text(input, "officeId"))); fields.put("mapping_revision", text(input, "accountMappingRevisionId"));
        fields.put("policy_revision", text(input, "policyRevisionId")); fields.put("calculator_build", text(input, "calculatorBuild"));
        fields.put("hel_payment_type_id", input.get("helPaymentTypeId").isNull() ? null : Long.parseLong(text(input, "helPaymentTypeId")));
        fields.put("config_json", json.write(input)); fields.put("released", false); fields.put("version", 0L);
        store.insert("configuration", scope, fields);
        for (JsonNode entry : input.get("accountMap")) store.insert("account_map", ReceivablesStore.key(scope, "map", text(entry, "accountKey")),
                Map.of("scope_key", scope, "account_key", text(entry, "accountKey"), "native_gl_id", Long.parseLong(text(entry, "nativeGlAccountId")),
                        "mapping_revision", text(input, "accountMappingRevisionId")));
        return capabilities(input.get("scope"), false);
    }
    public void validateProduct(long id) {
        ...
        } catch (IllegalArgumentException e) { throw new ReceivablesException("FINERACT_CAPABILITY_MISSING"); }
    }
    public JsonNode capabilities(JsonNode scope, boolean checkUser) {
        ...
        boolean ready;
        try { validateProduct(number(config, "product_id")); ready = true; }
        catch (ReceivablesException e) { ready = false; }
        ObjectNode result = json.object();
        result.put("calculationVersion", CALCULATION); result.put("productPolicyCode", POLICY); result.put("schemaVersion", "1");
        result.put("policyRevisionId", string(config, "policy_revision")); result.put("calculatorBuild", string(config, "calculator_build"));
        result.put("eventVersion", "flex.receivables.financial-event.v1"); result.put("productCode", "MNZL_PURCHASED_RECEIVABLE");
        result.put("scheduleCode", "MNZL_FIXED_RECEIVABLE"); result.put("chargeStrategyCode", "MNZL_NO_BORROWER_CHARGES"); result.put("accountingMode", "NONE");
        result.put("productConfigurationReady", ready); result.put("accountConfigurationReady", store.scoped("account_map", scopeKey(scope)).size() == ACCOUNTS.size());
        result.put("periodState", "OPEN"); result.put("businessDate", DateUtils.getBusinessLocalDate().toString());
    ...
    public void validateExecution(JsonNode command, Map<String, Object> config) {
        ...
        require(store.jdbc().queryForObject("select count(*) from m_mnzl_r_period where scope_key=? and status='CLOSED' and boundary_date>?", Long.class, scope, date) == 0,
                "PERIOD_CLOSED");
        String mode = text(command, "executionMode");
        if ("CURRENT".equals(mode)) {
            require(date.equals(DateUtils.getBusinessLocalDate()) && command.get("executionAuthorization").isNull(), "APPROVAL_SCOPE_CHANGED");
        } else {
            security.authenticatedUser().validateHasPermissionTo(("RECONSTRUCTION".equals(mode) ? "RECONSTRUCT" : "CORRECT") + "_MNZL_RECEIVABLES");
            ...
            Map<String, Object> recorded = store.require("authorization", ReceivablesStore.key(scope, "authorization", text(authorization, "authorizationId")));
        ...
        for (JsonNode approver : command.get("approverIds")) require(!approver.asText().equals(text(command, "actorId")), "APPROVAL_SCOPE_CHANGED");
    }
    @Transactional
    public JsonNode authorizeHistory(String request) {
        security.authenticatedUser().validateHasPermissionTo("CONFIGURE_MNZL_RECEIVABLES");
        JsonNode input = json.validate("nativeHistoricalAuthorization", request);
        String scope = scopeKey(input.get("scope")); store.lockConfiguration(scope);
        require(text(input, "approvedBy").equals(security.authenticatedUser().getId().toString()), "APPROVAL_SCOPE_CHANGED");
        LocalDate from = LocalDate.parse(text(input, "effectiveFrom")), through = LocalDate.parse(text(input, "effectiveThrough"));
        require(!through.isBefore(from), "INVALID_DATA");
        String key = ReceivablesStore.key(scope, "authorization", text(input, "authorizationId"));
        var fields = Map.<String, Object>of("scope_key", scope, "authorization_id", text(input, "authorizationId"), "scope_hash", text(input, "scopeHash"),
                "approved_by", text(input, "approvedBy"), "effective_from", from, "effective_through", through, "mode", text(input, "mode"), "revoked", false);
        var old = store.find("authorization", key);
        if (old == null) store.insert("authorization", key, fields);
        else require(string(old, "scope_hash").equals(text(input, "scopeHash")), "IDEMPOTENCY_CONFLICT");
        return input;
    }
}
