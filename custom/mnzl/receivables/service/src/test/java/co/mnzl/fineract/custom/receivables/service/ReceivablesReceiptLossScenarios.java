...
final class ReceivablesReceiptLossScenarios {
    ...
    private void scenario(boolean hel) throws Exception {
        ...
        JsonNode allLotsBefore = get(ROOT + "/developer-lots");
        JsonNode lotsBefore = allLotsBefore.path("items");
        String beforeWatermark = allLotsBefore.path("eventWatermark").asText();
        String boundaryQuery = "?businessDate=" + harness.today + "&boundarySide=AFTER_EVENTS&eventWatermark=";
        String sourcePath = ROOT + "/accounts/" + id;
        JsonNode sourcePositionBefore = get(sourcePath + "/position" + boundaryQuery + beforeWatermark);
        JsonNode sourceScheduleBefore = get(sourcePath + "/schedule" + boundaryQuery + beforeWatermark);
        JsonNode sourceLotsBefore = get(sourcePath + "/developer-lots" + boundaryQuery + beforeWatermark);
        ...
        String receiptWatermark = result.path("eventWatermark").asText();
        JsonNode sourcePositionAfter = get(sourcePath + "/position" + boundaryQuery + receiptWatermark);
        JsonNode sourceScheduleAfter = get(sourcePath + "/schedule" + boundaryQuery + receiptWatermark);
        JsonNode sourceLotsAfter = get(sourcePath + "/developer-lots" + boundaryQuery + receiptWatermark);
        assertThat(sourcePositionAfter).isEqualTo(sourcePositionBefore);
        assertThat(sourceScheduleAfter).isEqualTo(sourceScheduleBefore);
        assertThat(sourceLotsAfter.path("items")).isEqualTo(sourceLotsBefore.path("items"));
        evidence.put("beforeEventWatermark", beforeWatermark);
        evidence.set("sourcePositionBefore", sourcePositionBefore);
        evidence.set("sourcePositionAfter", sourcePositionAfter);
        evidence.set("sourceScheduleBefore", sourceScheduleBefore);
        evidence.set("sourceScheduleAfter", sourceScheduleAfter);
        evidence.set("sourceAccountLotsBefore", sourceLotsBefore);
        evidence.set("sourceAccountLotsAfter", sourceLotsAfter);
