...
/**
 * Regression tests for d84644ec2 (Fix cumulative declining balance first installment when period equals normal length).
 * Pre-fix the generator unconditionally treated the first period as extended/contracted and applied an extra-interest
 * adjustment, causing the first installment to drift even when the actual period was exactly one repayment period from
 * disbursement (e.g. Apr-1 -> May-1 with monthly repayments). The fix gates the adjustment on
 * {@code actualPeriodDays != idealPeriodDays}.
 */
@ExtendWith(MockitoExtension.class)
class CustomDecliningBalanceFirstPeriodEqualsNormalLengthTest {
    ...
    /** Disbursement on first-of-month so first period (Apr-1 -> May-1) is exactly one repayment period. */
    ...
    @Test
    void firstPeriodEqualsNormal_noExtraInterest() {
        // 30E/360 ideal first period: Apr-1 -> May-1 = 30 days. Actual first period also 30 days. So actualPeriodDays
        // equals idealPeriodDays and the extra-interest branch must not fire. Interest_1 must be 120k * 12% / 12 =
        // 1200.
        ...
        assertThat(first.interestDue()).as("first installment interest under normal-length first period")
                .isEqualByComparingTo(new BigDecimal("1200.00"));
    }

    @Test
    void firstPeriodEqualsNormal_principalDistributedNormally() {
        // With no extra-interest adjustment, principal must follow the standard EMI - interest = principal_due rule.
        // PMT for 120k, 12 monthly periods, 12% annual = 10661.85 (rounded). Interest_1 = 1200. So principal_1 ~=
        // 9461.85.
        LoanScheduleModel schedule = newGenerator().generate(MC, loanApplicationTerms(), Collections.emptySet(), holidayDetailDTO());

        LoanScheduleModelPeriod first = installment(schedule, 1);
        BigDecimal totalDue = first.principalDue().add(first.interestDue());
        // PMT(0.01 monthly, 12, -120000) ~= 10661.85 (HALF_EVEN to 2dp). Just check this is the standard EMI shape:
        assertThat(totalDue).as("first installment total = standard EMI").isBetween(new BigDecimal("10661.00"), new BigDecimal("10662.00"));
        assertThat(first.principalDue()).as("first installment principal").isGreaterThan(BigDecimal.ZERO);
    }

    @Test
    void firstPeriodEqualsNormal_matchesUpstreamReference() {
        // The post-fix generator must agree with the standard schedule for an unstubbed loan: total interest equals
        // PMT-based amortization (no surplus interest sneaks in). This catches a regression where the extra-interest
        // path is accidentally applied to the normal first period.
        LoanScheduleModel schedule = newGenerator().generate(MC, loanApplicationTerms(), Collections.emptySet(), holidayDetailDTO());
        ...
        // Total interest for 120k @ 12% / 12 monthly periods on a non-stubbed schedule rounds to 7942.04 (HALF_EVEN,
        // 2dp).
        // Lock the value to within a 1.00 EGP envelope so we tolerate end-of-loan principal rounding.
        assertThat(schedule.getTotalInterestCharged()).as("total interest unchanged from PMT amortization")
                .isBetween(new BigDecimal("7941.00"), new BigDecimal("7943.50"));
    }
