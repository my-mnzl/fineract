...
class RateChangeInExtendedFirstPeriodTest {
    ...
    @Test
    void rateChangeRecalculatesScheduleAndRestoresConfiguredFixedEmi() {
        BigDecimal fixedEmi = new BigDecimal("10661.85");
        LoanApplicationTerms terms = loanApplicationTerms(List.of(rateChange(LocalDate.of(2026, 5, 1), HIGHER_RATE)), BASE_RATE, fixedEmi);

        LoanScheduleModel schedule = newGenerator().generate(MC, terms, Collections.emptySet(), holidayDetailDTO());

        assertThat(schedule.getTotalPrincipalExpected()).isEqualByComparingTo(PRINCIPAL);
        BigDecimal secondInstallment = installment(schedule, 2).principalDue().add(installment(schedule, 2).interestDue());
        assertThat(secondInstallment).isNotEqualByComparingTo(fixedEmi);
        assertThat(terms.getFixedEmiAmount()).isEqualByComparingTo(fixedEmi);
    }
    ...
    private LoanApplicationTerms loanApplicationTerms(List<LoanTermVariationsData> termVariations, BigDecimal annualRate) {
        return loanApplicationTerms(termVariations, annualRate, null);
    }
    ...
    private LoanApplicationTerms loanApplicationTerms(List<LoanTermVariationsData> termVariations, BigDecimal annualRate,
        ...
        return LoanApplicationTerms.assembleFrom(CURRENCY.toData(), 12, MONTHS, 12, 1, MONTHS, null, INVALID, EQUAL_INSTALLMENTS,
                ...
                FIRST_REPAYMENT_DATE, FIRST_REPAYMENT_DATE, null, null, null, null, DISBURSED_ON, zeroTolerance, false, fixedEmiAmount,
                ...
                null, false, null, null, null, false, null, null, null, false, false);
    }
