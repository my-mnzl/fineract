...
class MnzlTransferFeeChargeForLoansTaskletTest {
    ...
    void oneTransferFailsOthersSucceed() {
        ...
        try (MockedStatic<DateUtils> mockedDateUtils = Mockito.mockStatic(DateUtils.class)) {
            ...
            assertThatThrownBy(() -> underTest.execute(stepContribution, chunkContext)).isInstanceOf(JobExecutionException.class);
        }
    ...
    @Test
    void missingLinkedSavingsAccountIsReportedWithoutAttemptingTransfer() {
        final LoanChargeData charge = nonInstallmentCharge(401L, 4L, new BigDecimal("10.00"), LocalDate.of(2026, 3, 10));
        when(loanChargeReadPlatformService.retrieveLoanChargesForFeePayment(ChargePaymentMode.ACCOUNT_TRANSFER.getValue(),
                LoanStatus.ACTIVE.getValue())).thenReturn(List.of(charge));
        when(accountAssociationsReadPlatformService.retriveLoanLinkedAssociation(4L)).thenReturn(null);
