...
/**
 * F.2 — Parameterized late-payment scenario over each prod product config.
 *
 * Drives DISBURSE + PAY×2 + SKIP + RUN_COB + PAY (catchup) + PAY×remaining via the simulator. The
 * MNZL_INTEREST_AND_PENALTIES charge strategy combined with MNZL_DUE_INSTALLMENTS COB is expected to apply a penalty
 * during RUN_COB; the catchup payment then clears installment 3 (principal + interest + penalty) and the loan
 * eventually closes.
 */
@Slf4j
public class MnzlLatePaymentParameterizedIT extends BaseLoanIntegrationTest {

    private static final DateTimeFormatter DATE_FMT = DateTimeFormatter.ofPattern("dd MMMM yyyy", Locale.ENGLISH);

    @ParameterizedTest(name = "late payment: {0}")
    @MethodSource("org.apache.fineract.integrationtests.mnzl.helpers.MnzlScenarioFixtures#prodProductConfigs")
    public void latePayment_skipPlusCobPlusCatchup_eventuallyCloses(String configName, Map<String, Object> productConfig) {
        runAt("01 January 2026", () -> {
            ...
            // Build the late-payment scenario.
            MnzlSimulationDriver.ScenarioBuilder s = driver.scenario("late_" + configName, productId).principal(principal).rate(rate)
                    .repayments(terms).disburseDate("01 January 2026").disburse("01 January 2026");
            // Pay installments 1 and 2 on time.
            s = s.pay(dueDateOf(periods, 1), totalDueOf(periods, 1));
            s = s.pay(dueDateOf(periods, 2), totalDueOf(periods, 2));
            // Skip installment 3 (no payment on its due date).
            s = s.skip(dueDateOf(periods, 3));
            // Advance and run COB shortly after the due date so MNZL_DUE_INSTALLMENTS can apply a penalty.
            String cobDate = addDaysFormatted(dueDateOf(periods, 3), 5);
            s = s.runCob(cobDate);
            // Catchup payment a few days later: pay enough to cover the EMI plus a small penalty buffer.
            String catchupDate = addDaysFormatted(dueDateOf(periods, 3), 7);
            s = s.pay(catchupDate, totalDueOf(periods, 3));
            // Finish the rest of the schedule on time.
            for (int i = 4; i <= terms; i++) {
                s = s.pay(dueDateOf(periods, i), totalDueOf(periods, i));
            }
            Map<String, Object> result = s.run();

            // Snapshot count: 1 disburse + 2 on-time + 1 skip + 1 cob + 1 catchup + (terms-3) remaining = 3 + terms.
