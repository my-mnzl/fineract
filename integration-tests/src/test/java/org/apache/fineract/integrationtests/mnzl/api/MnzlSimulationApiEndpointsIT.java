...
public class MnzlSimulationApiEndpointsIT extends BaseLoanIntegrationTest {
    ...
    private int simulatedClientCount() {
        String url = "/fineract-provider/api/v1/clients?search=Simulation%20Test%20Client&limit=1000&" + Utils.TENANT_IDENTIFIER;
    ...
    @Test
    @SuppressWarnings("removal")
    public void completedSimulation_removesTemporaryPortfolioGraph() {
        runAt(DISBURSEMENT_DATE, () -> {
            SavingsProductHelper.createSavingsProduct(new SavingsProductHelper().build(), requestSpec, responseSpec);
            Long productId = createMnzlProduct();
            MnzlSimulationDriver driver = new MnzlSimulationDriver(requestSpec, responseSpec);
            int clientsBefore = simulatedClientCount();

            Map<String, Object> result = validScenario(driver, productId, "g1_cleanup").run();

            assertThat(result.get("status")).isEqualTo("COMPLETED");
            assertThat(simulatedClientCount()).as("temporary loan, savings account, and client are deleted").isEqualTo(clientsBefore);
        });
    }
