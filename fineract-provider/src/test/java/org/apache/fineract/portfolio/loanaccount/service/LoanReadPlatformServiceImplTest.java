...
class LoanReadPlatformServiceImplTest {
    ...
    void fetchLoansForInterestRecalculation_passesBusinessDateAndStatusCodes() {
        ...
        assertThat(sqlCaptor.getValue()).contains("m_loan_repayment_schedule").contains("m_floating_rates_periods")
                .contains("interest_recalculation_enabled").contains("frp.from_date <= ?")
                .contains("l.interest_recalcualated_on < frp.from_date").doesNotContain("AND (frp.created_date >= ?");
    ...
    static LoanReadPlatformServiceImpl newService(JdbcTemplate jdbcTemplate) {
        return new LoanReadPlatformServiceImpl(jdbcTemplate, mock(PlatformSecurityContext.class), mock(LoanRepositoryWrapper.class),
                ...
                mock(CalendarReadPlatformService.class), mock(StaffReadService.class), mock(PaginationHelper.class),
                mock(PaymentTypeReadService.class), mock(FloatingRatesReadPlatformService.class), mock(LoanUtilService.class),
